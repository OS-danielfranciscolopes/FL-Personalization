...
class AggregateHeadStrategy(ServerInitializationStrategy):
    """Head Aggregation strategy implementation."""
    ...
    def configure_fit(
        ...
        """
        Configure the next round of training. Adds the global body to the aggregated global head.
        ...
        weights = [val.cpu().numpy() for _, val in self.model.body.state_dict().items()]

        # Add aggregated head parameters to initial body parameters
        weights.extend(parameters_to_weights(parameters=parameters))
    ...
    def aggregate_fit(
        ...
        model_keys = [k for k in self.model.state_dict().keys() if k.startswith("_head")]
